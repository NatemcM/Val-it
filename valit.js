		...
		//
			...
			if ( '[data-element^="valit-"]' ) // Check to see if the element has the data attribute
			{
				...
				if ( $(this).data('valit-tel') )
				{
					...
					if ( !validate_number( $(this).val() ) || $(this).val().length < defaults.telephone_length ) 
					{
						pass_validation = false;
						error_message  += valit_tel;
					}
				}
				...
				// Output the error message
					if ( !$(this).hasClass(defaults.input_error_class) ) // Only output the error once
					{
						...
						// Errors need to be stepped through
						valit_label.append('<' + defaults.wrap_error_message + ' class ="' + defaults.wrap_error_class +'">' + error_message + '</' + defaults.wrap_error_message + '> ');
					}
				} 
				else
				{
